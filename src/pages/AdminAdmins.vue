...
// Spremi / update admina
const handleSubmitAdmin = async () => {
  try {
    const dataToSend = { username: adminForm.value.username }
    if (adminForm.value.password) dataToSend.password = adminForm.value.password
    ...
    if (adminForm.value.id) {
      ...
      if (!adminForm.value.password) {
        $q.notify({ type: 'negative', message: 'Lozinka je obavezna' })
        return
      }
